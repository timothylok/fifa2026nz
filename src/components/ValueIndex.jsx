...
function getEloTrend(team, eloHistory) {
  const entries = eloHistory?.[team]
  if (!entries || entries.length === 0) return { delta: null, sparkline: [], direction: 'flat' }
  ...
  return { delta, sparkline, direction }
}
...
export default function ValueIndex({ teams }) {
  ...
  useEffect(() => {
    ...
    fetch('/data/elo_history.json')
      .then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json() })
      .then(setEloHistory)
      .catch(() => {}) // non-fatal — sparklines & movers just won't appear
  }, [])
  ...
  // Value-ranked rows (rank is stable — used for tier filter)
  const rows = useMemo(() => {
    if (!teams || !fairData) return []
    return buildValueRows(teams, fairData).map((r, i) => ({ ...r, valueRank: i + 1 }))
  }, [teams, fairData])

  // Elo trend per team (all teams in rows)
  const eloTrends = useMemo(() => {
    if (!eloHistory || rows.length === 0) return {}
    return Object.fromEntries(rows.map(r => [r.name, getEloTrend(r.name, eloHistory)]))
  }, [eloHistory, rows])

  // Elo movers: all 48 WC teams, sorted by 30-day Elo delta
  const eloMovers = useMemo(() => {
    if (!eloHistory) return null
    const withDelta = Object.keys(CONFED_MAP)
      .map(team => ({ team, delta: getEloTrend(team, eloHistory).delta }))
  ...
  const visibleRows = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      ...
      else if (sortCol === 'elo')    { av = eloTrends[a.name]?.delta; bv = eloTrends[b.name]?.delta }
      else if (sortCol === 'conf')   {
        const o = { High: 3, Medium: 2, Low: 1 }
        av = o[getConfidence(a.valueIndex)] ?? 0
        bv = o[getConfidence(b.valueIndex)] ?? 0
      }
      else { av = a.valueIndex; bv = b.valueIndex } // 'value' (default)
    ...
    <div className="space-y-8">
            ...
            return (
              ...
              >
                ...
                <div className="flex gap-4 text-xs text-[var(--muted)]">
                  <span>Model <span className="text-[var(--text)] font-mono font-medium">{fmt(row.modelProb)}</span></span>
                  <span>Mkt <span className="text-[var(--text)] font-mono font-medium">{fmt(row.bestFairImplied)}</span></span>
                </div>
                ...
                {trend?.delta != null && (
                  <div className="text-[11px] font-mono" style={{ color: trendColor(trend.direction) }}>
                    {trendArrow(trend.direction)} Elo {trend.delta >= 0 ? '+' : ''}{trend.delta} (30d)
                  </div>
                )}
              </div>
            )
      ...
      <div>
        <p className="text-xs text-[var(--muted)] mb-2">
          ...
          {' · '}click a row for bookmaker detail
        </p>

        {/* Column headers */}
        <div className="flex items-center gap-3 px-4 mb-1 text-[10px] uppercase tracking-wider font-semibold select-none">
          <span className="w-5 shrink-0 text-[var(--muted)]">#</span>
          <span className="w-5 shrink-0" />
          <span className="w-28 shrink-0 text-[var(--muted)]">Team</span>
          <SortBtn col="model"  label="Model"   sortCol={sortCol} sortDir={sortDir} onSort={handleSort} className="w-14 text-right shrink-0 justify-end" />
          <SortBtn col="market" label="Market"  sortCol={sortCol} sortDir={sortDir} onSort={handleSort} className="w-14 text-right shrink-0 justify-end" />
          <SortBtn col="value"  label="Value"   sortCol={sortCol} sortDir={sortDir} onSort={handleSort} className="w-16 text-right shrink-0 justify-end" />
          <SortBtn col="elo"    label="30d Elo" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} className="w-16 text-right shrink-0 justify-end" />
          <SortBtn col="conf"   label="Level"   sortCol={sortCol} sortDir={sortDir} onSort={handleSort} className="flex-1 text-right justify-end" />
        </div>
            ...
            return (
              <div key={row.name}>
                ...
                >
                  ...
                  <span className="text-sm font-medium w-28 shrink-0 truncate">{row.name}</span>
                  ...
                  <span className="text-xs font-mono w-16 text-right shrink-0"
                    ...
                    {trend?.delta != null
                      ? `${trendArrow(trend.direction)} ${trend.delta >= 0 ? '+' : ''}${trend.delta}`
                      : '—'}
                  </span>
      ...
      {eloMovers && (
        <div>
          <h3 className="text-sm font-semibold mb-3">Elo Momentum — last 30 days</h3>
